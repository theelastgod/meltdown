 ...
 *
 * This module imports nothing so the Workers can import it without dragging the devnet in.
 */
...
/**
 * What the deploy CLI checks before it spends gas on a real network (Stage 53): the relayer is not
 * a dev key, and the treasury is named and is not the relayer. The devnet defaults the treasury to
 * a separate dev key for the same reason; on a real chain there is no default at all.
 */
export function deployGuard(o: { relayerKey: string; relayerAddress: string; treasury: string | undefined }): { ok: true } | { ok: false; reason: string } {
