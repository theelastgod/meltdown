...
describe("chips and firmwares", () => {
  ...
  it("chips are validated: one per socket, right weapon, right socket, unlocked by rank", () => {
    ...
    expect(weaponKick.detail).not.toMatch(/is a STACK SMG chip/);
    expect(src).toMatch(/chip-weapon", detail: `\$\{c\.name\} IS A \$\{gun\(c\.weapon\)\} CHIP/);
    expect(src).not.toMatch(/chip-weapon", detail: `\$\{c\.name\} is a \$\{gun\(c\.weapon\)\} chip/);
    ...
    expect(noSockKick.detail).not.toBe('LEASE-BREAKER: no socket "BARREL"');
    ...
    expect(src).not.toMatch(/chip-socket", detail: `\$\{gun\(wid as WeaponId\)\}: no socket "\$\{socket\.toUpperCase\(\)\}"`/);
    ...
    const chipMap = validateLoadout({ ...DEFAULT_LOADOUT, chips: ["x"] }, owned, 50, ranks);
    expect(chipMap.errors.find((e) => e.rule === "chips-shape")!.detail).toBe("CHIPS MUST MAP WEAPON → SOCKET → CHIP ID");
    expect(chipMap.errors.find((e) => e.rule === "chips-shape")!.detail).not.toBe("chips must map weapon → socket → chip id");
    expect(src).toMatch(/detail: "CHIPS MUST MAP WEAPON → SOCKET → CHIP ID"/);
    expect(src).not.toMatch(/detail: "chips must map weapon → socket → chip id"/);
    const unknownGun = validateLoadout({ ...DEFAULT_LOADOUT, chips: { not_a_gun: { muzzle: "x" } } }, owned, 50, ranks);
    expect(unknownGun.errors.map((e) => e.rule)).toContain("chip-weapon");
    const unknownGunKick = unknownGun.errors.find((e) => e.rule === "chip-weapon" && /weapon/i.test(e.detail))!;
    expect(unknownGunKick.detail).toBe("UNKNOWN WEAPON not_a_gun");
    ...
    const unknownChip = validateLoadout({ ...DEFAULT_LOADOUT, chips: { lease_breaker: { muzzle: "nope" } } }, owned, 50, ranks);
    expect(unknownChip.errors.map((e) => e.rule)).toContain("unknown-chip");
    const unknownChipKick = unknownChip.errors.find((e) => e.rule === "unknown-chip")!;
    expect(unknownChipKick.detail).toBe("UNKNOWN CHIP nope");
    expect(unknownChipKick.detail).not.toBe("nope");
    expect(src).toMatch(/unknown-chip", detail: `UNKNOWN CHIP \$\{id\}`/);
    expect(src).not.toMatch(/unknown-chip", detail: id \}/);
    const unknownFw = validateLoadout({ ...DEFAULT_LOADOUT, firmware: { lease_breaker: "nope" } }, owned, 50, ranks);
    expect(unknownFw.errors.map((e) => e.rule)).toContain("unknown-firmware");
    const unknownFwIdKick = unknownFw.errors.find((e) => e.rule === "unknown-firmware")!;
    expect(unknownFwIdKick.detail).toBe("UNKNOWN FIRMWARE nope");
    expect(unknownFwIdKick.detail).not.toBe("nope");
    expect(src).toMatch(/unknown-firmware", detail: `UNKNOWN FIRMWARE \$\{id\}`/);
    expect(src).not.toMatch(/unknown-firmware", detail: id \}/);
  });
