...
describe("the FILE shop counts slots", () => {
  ...
  it("a slot the file already has is ALREADY OWNED, not already owned", () => {
    const src = readFileSync(new URL("../shared/endgame/rewrite.ts", import.meta.url), "utf8");
    expect(src).toMatch(/SLOT \$\{n\} ALREADY OWNED \(\$\{have\} SLOTS\)/);
  ...
  it("a waiting perf overlay is REPORT IN N S, not report in N s", () => {
    const src = readFileSync(new URL("../client/perf.ts", import.meta.url), "utf8");
    expect(src).toMatch(/REPORT IN \$\{Math\.max\(0, Math\.ceil\(this\.after - \(performance\.now\(\) - this\.startedAt\) \/ 1000\)\)\} S/);
...
describe("rewrite and the Wakelight shop", () => {
  ...
  it("ALIAS SLOT III's shop line is CRT, not a third", () => {
    const src = readFileSync(new URL("../shared/endgame/rewrite.ts", import.meta.url), "utf8");
    const line = COSMETICS.find((c) => c.id === "alias_3")!.line;
    expect(line).toBe("A THIRD");
    expect(line).not.toBe("a third");
  ...
  it("PRESET SLOT III's shop line is CRT, not a third", () => {
    const src = readFileSync(new URL("../shared/endgame/rewrite.ts", import.meta.url), "utf8");
    const line = COSMETICS.find((c) => c.id === "preset_3")!.line;
    expect(line).toBe("A THIRD");
    expect(line).not.toBe("a third");
  ...
  it("PRESET SLOT V's shop line is CRT, not a fifth", () => {
    const src = readFileSync(new URL("../shared/endgame/rewrite.ts", import.meta.url), "utf8");
    const line = COSMETICS.find((c) => c.id === "preset_5")!.line;
    expect(line).toBe("A FIFTH");
    expect(line).not.toBe("a fifth");
    expect(src).toMatch(/id: "preset_5", kind: "preset", name: "PRESET SLOT V", line: "A FIFTH"/);
    expect(src).not.toMatch(/id: "preset_5", kind: "preset", name: "PRESET SLOT V", line: "a fifth"/);
  });
});
