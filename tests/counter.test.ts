...
describe("the counter-ledger FILE line", () => {
  ...
  it("COUNTER-LEDGER CRT-cases the caught error, not TypeError: Failed to fetch", () => {
    ...
    expect(src).not.toMatch(/COUNTER-LEDGER: \$\{String\(e\)\}/);
  });

  it("a thrown sink burn CRT-cases FAILED, not DEEP WAKE SEASON 1 FAILED: Failed to fetch", () => {
    const src = readFileSync(new URL("../client/counter.ts", import.meta.url), "utf8");
    expect(src).toMatch(/\$\{label\} FAILED: \$\{crtPhrase\(/);
    expect(src).not.toMatch(/\$\{label\} FAILED: \$\{reason\}/);
  });
  ...
  it("NAME FAILED CRT-cases the caught error, not the provider's English", () => {
    ...
    expect(src).not.toMatch(/NAME FAILED: \$\{reason\}/);
  });
  ...
  it("SELL FAILED CRT-cases the caught error, not the provider's English", () => {
    ...
    expect(src).not.toMatch(/SELL FAILED: \$\{reason\}/);
  });

  it("BUY FAILED CRT-cases the caught error, not the provider's English", () => {
    const src = readFileSync(new URL("../client/counter.ts", import.meta.url), "utf8");
    expect(src).toMatch(/BUY FAILED: \$\{crtPhrase\(/);
    expect(src).not.toMatch(/BUY FAILED: \$\{reason\}/);
  });
