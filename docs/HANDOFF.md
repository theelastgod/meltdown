...
Written 2026-09-22, at Stage 447, for whoever works this next. It is deliberately not addressed to
...
| HEAD | Stage 447 on this branch (chip IS A CHIP, NOT) |
| Next stage number | **448** |
...
Stage 447 CRT-cased chip-socket `IS A … CHIP, NOT`. Firmware on the
wrong gun still kicks `is a … firmware`. Do not loosen fairness. Do not
