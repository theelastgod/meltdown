...
Written 2026-09-22, at Stage 329, for whoever works this next. It is deliberately not addressed to
...
| HEAD | Stage 329 on this branch (kill TTK is S) |
| Next stage number | **330** |
| CI | `verify` runs #202–#208 (Stages 172–178) all green; 179 not yet watched on CI |
| Unit tests | 1100 across 117 files, `npm test` |
...
Stage 329 CRT-cased kill TTK. `recordGhost` still suffixes RANGE times as `s`.
FLIP / LOCK IN on the node readout still uses `s`. Do not loosen fairness. Do not
