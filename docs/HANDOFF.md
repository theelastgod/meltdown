...
Stage 410 CRT-cased `EMPTY ALIAS`. Shop still returns
`slot N not owned (M slots)`. Do not loosen fairness. Do not plate viewmodel
or remote strips. The join line still prints the socket name — `probe:net`
asserts `probe-join-31`; leave it. Note `tests/seasonpass.test.ts` comments
quote the mixed-case form; update those if you CRT the string.
