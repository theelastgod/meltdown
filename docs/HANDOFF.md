...
Written 2026-09-22, at Stage 551, for whoever works this next. It is deliberately not addressed to
...
| HEAD | Stage 551 on this branch (HEADSHOT STAT_LINE) |
| Next stage number | **552** |
...
Stage 551 CRT-cased STAT_LINE `HEADSHOT`. Remaining STAT_LINE labels
still print mixed-case (`grenade`, `mantle`, `slide`, `throw`,
`range`). Title cards stay prose (probe:ship asserts them). Crew-code
