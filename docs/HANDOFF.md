...
Written 2026-09-22, at Stage 395, for whoever works this next. It is deliberately not addressed to
...
| HEAD | Stage 395 on this branch (NOT IN YOUR FILE) |
| Next stage number | **396** |
| CI | `verify` runs #202–#208 (Stages 172–178) all green; 179 not yet watched on CI |
| Unit tests | 1164 across 117 files, `npm test` |
...
Stage 395 CRT-cased `NOT IN YOUR FILE`. FILE still dims chip empty
`— none —`. Do not loosen fairness. Do not plate viewmodel or remote strips.
The join line still prints the socket name — `probe:net` asserts
