 ...
 * prestige. The WAKE items below are the only things that touch the chain, and none of them has
...
/** An on-chain cosmetic: an ERC-1155 token id, a WAKE price, a wear seed, and a palette the renderer tints with. Nothing else. */
...
const skin = (token: number, id: string, name: string, line: string, wake: number, wearSeed: number, tint: string): SkinDef => ({ token, id, name, line, wake, wearSeed, tint });
...
const onChain = (wake: number, tradable: boolean, randomness: MarketBlock["randomness"] = "none"): MarketBlock => ({ wake, onChain: true, tradable, randomness });
...
export function economyManifest(): EconomyItem[] {
  ...
  // Wakelight prestige: off-chain, never priced in WAKE, never a stat
  for (const c of COSMETICS) out.push({ id: c.id, kind: c.kind === "theme" ? "theme" : "cosmetic", mechanical: null, market: null });
  for (const s of SKINS) out.push({ id: s.id, kind: "cosmetic", mechanical: null, market: onChain(s.wake, true, "wear_seed") });
  ...
  out.push({ id: "rewrite_certificate", kind: "rewrite_certificate", mechanical: null, market: { wake: null, onChain: true, tradable: false, randomness: "none" } });
  return out;
}
