...
export function completeContract(a: Account, id: string, testimony: Testimony): { ok: boolean; reason?: string; reward?: Reward } {
  ...
  a.ledger.push(`${m.kind === "mission" ? "MISSION" : "GIG"} CLOSED · ${m.title}${m.reward.scrip ? ` · +${m.reward.scrip} SCRIP` : ""}${m.reward.protocol ? ` · PROTOCOL ${protocolById(m.reward.protocol)?.name ?? m.reward.protocol}` : ""}${m.reward.weapon ? ` · WEAPON ${WEAPONS[m.reward.weapon as WeaponId]?.name ?? m.reward.weapon.replace(/_/g, " ").toUpperCase()}` : ""}`);
