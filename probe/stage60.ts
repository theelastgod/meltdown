...
async function main(): Promise<void> {
  ...
  try {
    ...
    const chrome = await pg.evaluate(async () => {
      ...
      return { width: H.width, rack: out["rack"]!, nades: out["nades"]!, alert: out["alert"]!, mission: out["mission"]!, status: out["status"]!, ammobar: out["status .bar.ye"]!, alertText: hud.querySelector(".alert")!.textContent, line: { scroll: line.scrollWidth, client: line.clientWidth, text: line.textContent ?? "", overflow: getComputedStyle(line).textOverflow }, rackLabels: [...hud.querySelectorAll(".rack span")].map((e) => (e.textContent ?? "").replace(/\d+$/, "").trim()) };
    });
