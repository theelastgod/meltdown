...
async function openClient(browser: Awaited<ReturnType<typeof chromium.launch>>, room: string, name: string, seed: number): Promise<Page> {
  ...
  const url = `http://127.0.0.1:${VITE_PORT}/?norender=1&net=ws://127.0.0.1:${HOST_PORT}/room/${room}&name=${name}&lat=${RTT / 2}&jitter=8&loss=${LOSS}&seed=${seed}`;
...
async function main(): Promise<void> {
  ...
  try {
    ...
    await E.a.screenshot({ path: `${OUT}/stage2-alpha.png` });
    await E.b.screenshot({ path: `${OUT}/stage2-bravo.png` });
    await E.a.close();
    await E.b.close();
    ...
    await a2.goto(`http://127.0.0.1:${VITE_PORT}/?norender=1&net=ws://127.0.0.1:${HOST_PORT}/room/${room2}?lagcomp=0&name=ALPHA&lat=${RTT / 2}&jitter=8&loss=${LOSS}&seed=11`, { waitUntil: "load" });
