...
async function main(): Promise<void> {
  ...
  try {
    ...
    check("the menu tells a thumb how to use it rather than naming keys a phone has not got, on its footer and on the settings line", menuSaid.foot.startsWith("TAP A LINE TO CHOOSE") && !menuKeys.test(menuSaid.foot.replace(menuSaid.build, "")) && menuSaid.settings === "TAP [−] [+] · APPLIED LIVE · KEPT IN THIS BROWSER" && !menuKeys.test(menuSaid.settings) && menuSaid.chips > 0, `footer "${menuSaid.foot}" \u00b7 settings line "${menuSaid.settings}" \u00b7 ${menuSaid.chips} adjust chips on ${menuSaid.rows} rows \u00b7 back to ${menuSaid.screen}`);
