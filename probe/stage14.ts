...
async function main(): Promise<void> {
  ...
  try {
    ...
    // and at 640 (Stage 111), where the band cannot hold the panel beside the status panel at all:
    // the panel takes the second row, under the status panel and short of the map
    await a.setViewportSize({ width: 640, height: 360 });
    const tight = await a.evaluate(async () => {
      await new Promise((r) => requestAnimationFrame(() => requestAnimationFrame(r)));
      const hud = document.getElementById("hud")!;
      const H = hud.getBoundingClientRect();
      const out: Record<string, { left: number; right: number; top: number; bottom: number }> = {};
    ...
    const moments = await c.evaluate(async () => {
      ...
      return { carried, pickLine, claim: audio0["claim"] ?? 0, flips: audio0["nodeFlip"] ?? 0, drops: (audio1["dropClaims"] ?? 0) - (audio0["dropClaims"] ?? 0), dropLine: [...document.querySelectorAll("#hud .log div")].map((d) => d.textContent ?? "").find((l) => /DROPPED WHERE YOU FELL/.test(l)) ?? "", after: run?.carried ?? -1, onStreet: run?.claims.some((cl) => cl.dropped) ?? false, alive: p.alive };
    });
    // The shipped line is pinned whole, not matched by substring. This scenario carries exactly one
    // unit, and Stage 346 made one unit read "1 UNIT" rather than "1 UNITS" — a /UNITS DROPPED/
