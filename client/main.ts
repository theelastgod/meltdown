...
export interface GameHook {
  ...
  state: () => {
    ...
    life: { clock: number; crowd: number; sample: { x: number; z: number }[]; tram: number | null; tramNear: boolean; tramDist: number; pa: string[]; steam: boolean; ads: number; adRedraws: number; adFed: number; adCarry: number; adFrames: number[]; ship: { x: number; y: number; z: number }; blinkers: number; flicker: boolean };
  };
...
window.__game = {
  ...
  state: () => ({
    ...
    life: (() => {
      ...
      return { clock: L.clock, crowd: L.crowd?.count ?? 0, sample: L.crowd?.sample(8) ?? [], tram: L.tram ? L.tram.position : null, tramNear: !!L.tram?.near, tramDist: L.tram ? L.tram.distanceTo(eye) : Infinity, pa: game.cityLog.slice(), steam: !!L.steam, ads: L.ads?.group.children.length ?? 0, adRedraws: L.ads?.redraws ?? 0, adFed: L.ads?.fed ?? 0, adCarry: L.ads?.carry ?? 0, adFrames: L.ads?.recent(600) ?? [], ship: L.sky.shipPos, blinkers: L.sky.blinkers, flicker: !!game.renderer.signFlicker };
    })(),
  }),
  file: () => game.file.view(),
