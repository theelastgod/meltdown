...
export class GhostFile {
  ...
  counterHtml(): string {
    ...
    const sinkBlock = v?.linked && prices
      ...
      : "";
    const runBlock = v?.linked ? `<div class="ln">THE RUN · TODAY <b>${run.banked}</b>/${RUN_DAILY_CAP} · OWED <b>${run.owed}</b> UNITS · PAID ${Number(run.paid).toFixed(2)} $CAPITAL ${run.owed > 0 ? `<span class="btn" data-act="payout">[WITHDRAW TO WALLET]</span>` : ""}${v.runGate ? "" : ` <span class="dim">· below Depth ${RUN_DEPTH} the run pays Scrip</span>`}<span class="dim"> · units settle nightly at up to ${MAX_CAPITAL_PER_UNIT} $CAPITAL each, out of the day's emission; once a day has settled its units are a PRIZE below, not a withdrawal</span></div>` : `<div class="ln dim">THE RUN pays the wallet: link one and the units you bank at a gate settle into $CAPITAL.</div>`;
